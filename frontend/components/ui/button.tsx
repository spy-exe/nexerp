  ...
  ({ children, className, disabled, isLoading = false, variant = "primary", size = "md", ...props }, ref) => {
    return (
      <button
        ref={ref}
        className={cn(
          "inline-flex items-center justify-center rounded-lg text-sm font-semibold transition-all duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-[var(--bg)] disabled:cursor-not-allowed disabled:opacity-50",
          size === "md"   && "h-9 px-4",
          ...
          variant === "outline" &&
            "border border-[color:var(--border)] bg-transparent text-[var(--fg)] hover:border-[color:var(--border-hover)] hover:bg-[var(--bg-muted)]",
          variant === "danger" &&
